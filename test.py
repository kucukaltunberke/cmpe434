...
def main(robot_type=RobotType.circle):
    ...
    dwa_config.max_speed = 3.0  # [m/s]
    dwa_config.min_speed = -.5 # [m/s]
    dwa_config.max_yaw_rate = 40.0 * math.pi / 180.0  # [rad/s]
    dwa_config.max_accel = 3  # [m/ss]
    dwa_config.max_delta_yaw_rate = 40.0 * math.pi / 180.0  # [rad/ss]
    dwa_config.v_resolution = 0.01  # [m/s]
    dwa_config.yaw_rate_resolution = 0.2 * math.pi / 180.0  # [rad/s]
    dwa_config.dt = 0.01  # [s] Time tick for motion prediction
    dwa_config.predict_time = 2.0  # [s]
    dwa_config.to_goal_cost_gain = 6
    dwa_config.speed_cost_gain = 1
    dwa_config.obstacle_cost_gain = 0
    dwa_config.robot_stuck_flag_cons = 0.001  # constant to prevent robot stucked
    dwa_config.robot_radius = 0.6  # [m] for collision check

    kp_t, kd_t, ki_t, dt = 0, 0, 0, 0.1
    pid_controller_throttle = PIDcontroller(kp_t, kd_t, ki_t, dt)
    

    kp_s, kd_s, ki_s, dt =1.5, .05, 0, 0.1
    pid_controller_steering = PIDcontroller(kp_s, kd_s, ki_s, dt)
    ...
    while True:
        ...
        path_id_selected = min(path_id + 3, max_path_id)

        target_selected=[target_x_list[path_id_selected],target_y_list[path_id_selected]]
        max_speed   = dwa_config.max_speed
        pred_time   = dwa_config.predict_time
        robot_r     = dwa_config.robot_radius
        max_reach   = max_speed * pred_time + robot_r

        # 4) Filter obstacles by distance to robot
        curr_pos = state[:2]     

        dists       = np.linalg.norm(ob_xy - curr_pos, axis=1)
        ob_in_range = ob_xy[dists <= max_reach]

        u, predicted_trajectory = dwa_control(state, dwa_config, target_selected, ob_in_range)
        v_cmd, omega_cmd = u


        # current_yaw = state[2]
        # current_velocity = state[3]
        # velocity_control = pid_controller_throttle.update(v_cmd, current_velocity)



        # print(current_velocity,v_cmd,velocity_control)



        trajectory = np.vstack((predicted_trajectory, state))  # store state history
        ...
        # print(u)
        # wheel_speed = v_cmd / wheel_R                      # rad/s
        # speed      = max(abs(state[3]), 1e-3)
        # delta      = math.atan2(L * omega_cmd, speed)      # rad

        # steering_control=pid_controller_steering.update(delta,0)
        ...
        # velocity.ctrl = velocity_control
        # steering.ctrl=np.clip(steering_control,-4,4)
